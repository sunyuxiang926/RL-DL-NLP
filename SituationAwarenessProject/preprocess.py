...
def get_args():
    ...
    parser.add_argument('--data', default='your data path', help='path to COCO datasets')
    parser.add_argument('--dest-dir', default='newdestination', help='destination dir')

    parser.add_argument('--train-image', default='train', help='relative path to train images')
    parser.add_argument('--valid-image', default='val', help='relative path to validation images')
    parser.add_argument('--test-image', default='test', help='relative path to test images')

    parser.add_argument('--train-caption', default='captions_train.json', help='train captions')
    parser.add_argument('--valid-caption', default='captions_val.json', help='validation captions')
    parser.add_argument('--test-caption',  default='captions_test.json', help='test captions')
...
def extract_features(args, model, image_dataset, output_dir):
    ...
    for caption_ids, image_paths, sample in progress_bar:
        # print(sample.shape) # sample.shape [10,3,224,224]
        image_features = model(utils.move_to_cuda(sample)) # shape [10,512,14,14]
        # print(image_features.shape)
        image_features = image_features.view(*image_features.size()[:-2], -1) # shape [10,512,196]
        # print(image_features.shape)
        # B x C x (H x W) -> B x (H x W) x C //  B = batch_size C = 512 H * W = 14 * 14
        image_features = image_features.transpose(1, 2) # shape [10,196,512]
        # print(image_features.shape)
        image_features = image_features.cpu().detach().numpy().astype(np.float32) # shape (10,196,512)

        for id, image_path, features in zip(caption_ids.cpu().numpy().astype(np.int32), image_paths, image_features):
            # features.shape (196,512)
